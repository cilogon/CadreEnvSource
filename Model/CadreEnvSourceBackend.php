...
class CadreEnvSourceBackend extends OrgIdentitySourceBackend {
  public $name = "CadreEnvSourceBackend";
  ...
  protected function resultToOrgIdentity($result) {
    ...
    if(empty($orgdata['Name'][0]['given'])) {
      ...
      $orgdata['Name'][0]['family'] = _txt('pl.cadreenvsource.name.unknown');
    }
...
//      'sorid'   => IdentifierEnum::SORID,
    ...
    // Use the scoped affiliation string if present to add
    // AdHocAttributes.

    if(!empty($result['env_affiliation_scoped'])) {
      $orgdata['AdHocAttribute'][0]['tag'] = 'affiliation_scoped';
      $orgdata['AdHocAttribute'][0]['value'] = $result['env_affiliation_scoped'];

      $scopedAffiliations = explode(';', $result['env_affiliation_scoped']);
      $scoped = array();
      foreach($scopedAffiliations as $sa) {
        $ret = explode('@', $sa);
        $scoped[] = $ret[0];
      }

      $orgdata['AdHocAttribute'][1]['tag'] = 'affiliation';
      $orgdata['AdHocAttribute'][1]['value'] = implode(';', $scoped);
    }
  ...
  public function retrieve($id) {
    ...
    if(empty($this->pluginCfg['env_identifier_sorid'])) {
      throw new RuntimeException(_txt('er.cadreenvsource.sorid.cfg'));
    }
    
    $sorid = getenv($this->pluginCfg['env_identifier_sorid']);
    
    if(!$sorid) {
      throw new RuntimeException(_txt('er.cadreenvsource.sorid', array($this->pluginCfg['env_identifier_sorid'])));
    }
    
    if($sorid != $id) {
      throw new RuntimeException(_txt('er.cadreenvsource.sorid.mismatch', array($this->pluginCfg['env_identifier_sorid'])));
    }
    
    // Note the controller must $use this for it to be available, apparently
    $CadreEnvSource = ClassRegistry::init("CadreEnvSource.CadreEnvSource");
    
    $attrs = $CadreEnvSource->availableAttributes();
    $values = array();
