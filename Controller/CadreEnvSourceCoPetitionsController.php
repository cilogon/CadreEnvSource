...
class CadreEnvSourceCoPetitionsController extends CoPetitionsController {
  // Class name, used by Cake
  public $name = "CadreEnvSourceCoPetitions";

  public $uses = array("CoPetition",
                       "OrgIdentitySource",
                       "CadreEnvSource.CadreEnvSource",
                       "CadreEnvSource.CadreEnvSourceBackend");
  ...
  protected function execute_plugin_collectIdentifierIdentify($id, $oiscfg, $onFinish, $actorCoPersonId) {
    ...
    $args = array();
    $args['conditions']['CadreEnvSource.org_identity_source_id'] = $oiscfg['OrgIdentitySource']['id'];
    $args['contain'] = false;
    
    $cfg = $this->CadreEnvSource->find('first', $args);
    ...
    if(empty($cfg['CadreEnvSource']['env_identifier_sorid'])) {
      throw new RuntimeException(_txt('er.cadreenvsource.sorid.cfg'));
    }
    
    $sorid = getenv($cfg['CadreEnvSource']['env_identifier_sorid']);
    
    if(!$sorid) {
      throw new RuntimeException(_txt('er.cadreenvsource.sorid', array($cfg['CadreEnvSource']['env_identifier_sorid'])));
    }
    ...
    try {
      ...
      $this->flagDuplicate($id,
                           ...
                           $actorCoPersonId,
                           _txt('er.cadreenvsource.sorid.dupe', array($sorid, $oiscfg['OrgIdentitySource']['description'])),
                           $cfg['CadreEnvSource']['redirect_on_duplicate']);
    }
    catch(Exception $e) {
    ...
    if($cfg['CadreEnvSource']['duplicate_mode'] != CadreEnvSourceDuplicateModeEnum::SORIdentifier) {
      ...
      if($cfg['CadreEnvSource']['duplicate_mode'] == CadreEnvSourceDuplicateModeEnum::LoginIdentifier) {
        ...
        foreach($newOrgIdentifiers as $identifier) {
          ...
          if($count > 0) {
            ...
            $this->flagDuplicate($id,
                                 $newOrgId,
                                 $actorCoPersonId,
                                 _txt('er.cadreenvsource.dupe', array($sorid)),
                                 $cfg['CadreEnvSource']['redirect_on_duplicate']);
          }
        }
      }
